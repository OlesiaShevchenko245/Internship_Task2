...
public class ObservationService {
    ...
    public ObservationService(AstronomicalObservationRepository observationRepository, AuthorRepository authorRepository) {
    ...
    public ObservationDTO update(Long id, ObservationDTO dto) {
        ...
        AstronomicalObservation updated = observationRepository.save(observation);
    ...
    public ObservationListResponse list(ObservationFilterRequest request) {
        ...
        Page<AstronomicalObservation> page = observationRepository.findByFilters(
                request.getAuthorId(),
                request.getName(),
        ...
        List<ObservationListDTO> list = page.getContent().stream()
                .map(this::toListDTO)
    ...
    public byte[] generateReport(ObservationFilterRequest request) throws IOException {
        List<AstronomicalObservation> observations = observationRepository.findAllByFilters(
                request.getAuthorId(),
                request.getName(),
                request.getStartTime()
        );
    ...
    private byte[] createExcelReport(List<AstronomicalObservation> observations) throws IOException {
        ...
        String[] headers = {"ID", "Name", "Description", "Observation Time", "Author", "Celestial Objects"};
        ...
        for (AstronomicalObservation obs : observations) {
            ...
            row.createCell(4).setCellValue(obs.getAuthor().getFirstName() + " " + obs.getAuthor().getLastName());
            row.createCell(5).setCellValue(obs.getCelestialObjects() != null ? String.join(", ", obs.getCelestialObjects()) : "");
        }
    ...
    private ObservationListDTO toListDTO(AstronomicalObservation observation) {
        return new ObservationListDTO(
                ...
                observation.getAuthor().getFirstName() + " " + observation.getAuthor().getLastName()
        );
    }
}
